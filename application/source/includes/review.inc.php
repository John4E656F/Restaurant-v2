...
if (isset($_GET['page_id'])) {
    ...
    if(isset($_POST['submit'])) {
        ...
        $stmt = $pdo->prepare("INSERT INTO review (page_id, name, email, content, image, rating) 
                VALUES ('$page_id','$name', '$email', '$content', '$image', $rating);");
    ...
    $stmt = $pdo->prepare('SELECT * FROM review WHERE page_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['page_id']]);
    $review = $stmt->fetchAll(PDO::FETCH_ASSOC);

    //Get the overall rating and total amount of reviews
    $stmt = $pdo->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM review WHERE page_id = ?');
    $stmt->execute([$_GET['page_id']]);
...
<div class="overall_rating">
    ...
    <span class="stars"><?=str_repeat('&#9733;', round($reviews_info['overall_retaing']))?></span>
